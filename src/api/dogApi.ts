...
import { ApiResponse, DogApiBreed } from '../types/dog';
...
export const fetchAllBreeds = async (): Promise<DogApiBreed[]> => {
  ...
  const firstPage = await apiClient.get<ApiResponse>('/breeds?page[number]=1');
  ...
  if (totalPages > 1) {
    ...
    for (let i = 2; i <= totalPages; i++) {
      promises.push(apiClient.get<ApiResponse>(`/breeds?page[number]=${i}`));
    }
